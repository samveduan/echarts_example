...
;! function() {
	...
	layer.prompt = function(a, c) {
		...
		var d, e = 2 == a.formType ? '<textarea class="layui-layer-input">' + (a.value || "") + "</textarea>" : function() {
			//var input_str='<input type="' + (1 == a.formType ? "password" : "text") + '" class="layui-layer-input" placeholder="' + (a.placeholder || "") + '" value="' + (a.value || "") + '">';
			var input_str='<input type="' + (1 == a.formType ? "password" : "text") + '" class="layui-layer-input" placeholder="' + (a.placeholder || "") + '" style="font-size:12px;" />';
			return input_str;
		}();
		return layer.open($.extend({
			...
			success : function(a) {
				...
				d.blur();//失去光标
				//绑定回车事件
				a.find(".layui-layer-input").bind("keydown",function(event){
		            if (event.keyCode == 13){
